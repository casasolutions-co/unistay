...
function CityPicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const suggestions = q.length > 0
    ? CITY_POPULAR.filter(c => c.toLowerCase().startsWith(q))
    : CITY_POPULAR;
        ...
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 200, background: '#fff', border: '1.5px solid #e6e2ef', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,.12)', overflow: 'hidden' }}>
          ...
          {suggestions.map(name => (
            <button key={name} type="button"
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 14px', background: query === name ? '#f3effe' : 'none', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 600, color: query === name ? '#6d28d9' : '#1c1530', textAlign: 'left', fontFamily: 'inherit', transition: 'background .1s' }}
              onMouseEnter={e => { if (query !== name) e.currentTarget.style.background = '#f8f6ff'; }}
              onMouseLeave={e => { if (query !== name) e.currentTarget.style.background = 'none'; }}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={query === name ? '#6d28d9' : '#9a94a8'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function ListCasaApartment() {
  ...
  async function uploadFiles(files: FileList) {
    ...
    await Promise.all(
      fileArray.map(async (file, i) => {
        ...
        try {
          const res = await fetch('/api/casa-photos', { method: 'POST', body: fd });
  ...
  async function handlePublish() {
    ...
    setPublishError('');
    try {
      const res = await fetch('/api/casa-listings', {
        method: 'POST',
  ...
  return (
    <div className={styles.page}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, fontSize: 13, fontWeight: 700 }}>
        ...
        <span style={{ color: '#1c1530' }}>List Apartment</span>
      </div>
      ...
      <div className={styles.body}>
        ...
        <div className={styles.formCol}>
          ...
          <div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 32, lineHeight: 1.04, letterSpacing: '-0.03em', margin: 0, color: 'var(--text)' }}>List a CASA apartment</h1>
            <p style={{ fontSize: 15, color: 'var(--text-muted)', margin: '10px 0 0' }}>Fill in the details below. It publishes immediately under the CASA badge — no review queue.</p>
          </div>
